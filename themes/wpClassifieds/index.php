...
if ($advs){//advanced search form
  echo '<div>'.advancedSearchForm().'</div>';
}
  ...
  if (isset($location))
	...
	if ($resultSearch) {
		foreach ( $resultSearch as $row ) {
          ...
          <?php echo " [".$postCoursenumber."] ".$postTitle;?>
        </a><?php echo " by ".$postAuthor?></h2>
...
<div class="pagination">
  ...
  <?php //page numbers echo $_SERVER["REQUEST_URI"];
      ...
      if (strlen(cG("s"))>=MIN_SEARCH_CHAR)
        ...
        if ($i == $page)
          echo "<span class='current'>$i</span>";//not printing link current page
        else
          echo "<a class='page' title='$pag_title$i' href='".SITE_URL."$pag_url$i'>$i</a>";//print the link
		  }
		  if ($page < $total_pages){
...
<?php    
  ...
  <div id="welcome">
    ...
    <p>
      ...
      <strong>SAFETY</strong>  trade with fellow students and inspect a book before paying
    </p>
    <p>
...
All transactions occur outside Penn Book Bazaar. You can contact a seller either by sending her a message in the contact box (under each book) or emailing her directly. Responses are sent to your Penn email address. You should confirm price and discuss the logistics of the exchange in the following correspondence.
    ...
    <p><strong style="color:#F04949;">Selling a Book</strong><br/>
      <a href="<?php echo SITE_URL; ?>/publish.htm">Post a book for "offer"</a>. Fill in the details of the book, including authors, course information, ISBN, and a description of condition. Submit, and all you have to do is wait for buyers to contact you via email.
    </p>
  </div>
<?php } ?>
