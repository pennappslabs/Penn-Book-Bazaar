...
//starting variables for the app
	...
	$ocdb = new phpMyDB(DB_USER, DB_PASS, DB_NAME,DB_HOST,DB_CHARSET);//GLOBAL database variable connection
	if (DEBUG) $ocdb->setDebug(true);//db debug
	if (CACHE_ACTIVE) $ocdb->setCache(true,CACHE_EXPIRE,CACHE_DATA_FILE);//cache for DB
//end
	
//get possible parameters for the app	
  	if (cG("category")!="") $currentCategory=cG("category");
...
//data for the item
	if (isset($idItem)) {
		...
		if (mysql_num_rows($result)){
			$row=mysql_fetch_assoc($result);
			$itemid=$row["idPost"];
			$itemCoursenumber=$row["coursenumber"];
			$itemCoursename=$row["coursename"];
			$itemTitle=$row["title"];
			$itemAuthor=$row["author"];
			$itemISBN=$row["isbn"];
    ...
    elseif(strpos($_SERVER["SCRIPT_NAME"], "feed-rss.php")>0){
	        ...
	        $query="SELECT p.idPost,p.title,p.description,p.insertDate,p.place,
					            c.Name category,c.idCategoryParent,p.type,p.price, c.friendlyName,p.hasImages,
					            (select friendlyName from ".TABLE_PREFIX."categories where idCategory=c.idCategoryParent limit 1) parent
	...
	elseif( ( strpos($_SERVER["SCRIPT_NAME"], "index.php")>0 && !strpos($_SERVER["SCRIPT_NAME"], "admin")>0 ) || ( strpos($_SERVER["SCRIPT_NAME"], "listing.php")>0) ){
		...
		if (isset($location)){//there's a location set
            if ($location!=0) $filter.= " and p.idLocation=$location ";//location
		}
		...
		if (strlen(cG("s"))>=MIN_SEARCH_CHAR){
			if (strpos(cG("s"), ",")>0){//search for location
				$search=explode(",",cG("s"));
				$filter.= " and (p.title like '%$search[0]%' or  p.description like '%$search[0]%' or p.coursenumber like '%$search[0]%' or p.coursename like '%$search[0]%' or p.place like '%$search[1]%') ";
			}
			else $filter.= " and (p.title like '%".cG("s")."%' or p.author like '%".cG("s")."%' or  p.description like '%".cG("s")."%' or p.isbn like '%".cG("s")."%' or p.coursenumber like '%".cG("s")."%' or p.coursename like '%".cG("s")."%') ";
		}
		//end search
		...
		if (strlen(cG("author"))>=MIN_SEARCH_CHAR){
			$filter.= " and p.author like '%".cG("author")."%' ";
			$advs=true;
		}
		if (strlen(cG("isbn"))>=MIN_SEARCH_CHAR){
			$filter.= " and p.isbn like '%".cG("isbn")."%' ";
			$advs=true;
		}
		if (strlen(cG("coursenumber"))>=MIN_SEARCH_CHAR){
			$filter.= " and p.coursenumber like '%".cG("coursenumber")."%' ";
			$advs=true;
		}
		if (strlen(cG("coursename"))>=MIN_SEARCH_CHAR){
			$filter.= " and p.coursename like '%".cG("coursename")."%' ";
			$advs=true;
		}
		...
		//end adv search
			...
			$query="SELECT p.idPost,p.coursenumber,p.title,p.author,p.isbn,p.coursename,p.description,p.insertDate,
							c.Name category,c.friendlyName fcategory,c.idCategoryParent,p.type,p.price,p.name,p.password,p.hasImages,
							(select friendlyName from ".TABLE_PREFIX."categories where idCategory=c.idCategoryParent limit 1) parent
							FROM ".TABLE_PREFIX."posts p
								inner join ".TABLE_PREFIX."categories c
						on c.idCategory=p.idCategory
