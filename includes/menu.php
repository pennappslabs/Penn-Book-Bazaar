...
function generatePopularCategories(){//popular categories displayed in the home
	...
	$result=$ocdb->getRows($query,"assoc","APP");
	
	$popularCategories="<b>"._("Popular")."</b>";
	foreach ( $result as $category ) {
		$name=$category["name"];
		$fcategory=$category["friendlyName"];
		$cont=$category["cont"];
...
function generatePostType($currentCategory,$type){ //shows types in the right side
	...
	if (!isset($currentCategory)) $currentCategory=_("All");
    
    $offerUrl = typeURL(TYPE_OFFER,$currentCategory);
    $needUrl = typeURL(TYPE_NEED,$currentCategory);
    if (!$currentCategory==_("All"))
	   $allUrl = catURL($currentCategory,$selectedCategory);
    
	if ($type==TYPE_OFFER&&isset($type)) echo "<b>".ucwords(_("offer"))."</b>";
	else echo "<a title=\""._("offer")." $currentCategory\" href=\"".SITE_URL."$offerUrl\">".ucwords(_("offer"))."</a>";
	
	if ($type==TYPE_NEED&&isset($type)) echo SEPARATOR."<b>".ucwords(_("need"))."</b>";
	else echo SEPARATOR."<a title=\""._("need")." $currentCategory\" href=\"".SITE_URL."$needUrl\">".ucwords(_("need"))."</a>";
	
	if (isset($type)) echo SEPARATOR."<a href=\"".SITE_URL."$allUrl\">"._("All")."</a>";
	else echo SEPARATOR."<b>"._("All")."</b>";
}
////////////////////////////////////////////////////////////
...
function generatePopularItems($days=7,$limit=5,$idCategory){//displays the top X limit popular items from the last X days, possible by category
	if (COUNT_POSTS){
	    ...
	    $query="select P.*,C.friendlyName,(select friendlyName from ".TABLE_PREFIX."categories where idCAtegory=C.idCategoryParent Limit 1) parent
                    from ".TABLE_PREFIX."categories C inner join
			            (SELECT count(P.idPost) cont,P.idPost,P.title,P.type,P.idCategory FROM ".TABLE_PREFIX."posts P
...
////////////////////////////////////////////////////////////
function totalAds($idCategory,$days=0){//return the total number of posts
	global $ocdb;
	...
	return $ocdb->getValue($query,"APP");
}
////////////////////////////////////////////////////////////
function totalViews($idCategory,$days=0){//return the total number of posts views
	global $ocdb;
	if (!isset($idCategory)) $idCategory="all";
	if ($idCategory!="all"){
		$filter =" and (P.idCategory=$idCategory or P.idCategory in (select idCategory from ".TABLE_PREFIX."categories where idCategoryParent=$idCategory))";
