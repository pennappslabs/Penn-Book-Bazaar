...
if ($_POST) {
    ...
    if ($account->exists) {
        ...
        if (strpos($url, "?")) {
            $url .= '&amp;account='.$email.'&amp;token='.$token.'&amp;action=confirm';
        } else {
            $url .= '?account='.$email.'&amp;token='.$token.'&amp;action=confirm';
        }
        ...
        $show_form = false;
        echo "<div id='sysmessage'>"._("Instructions to confirm your account has been sent").". "._("Please, check your email")."</div>"; //email sent notice
    } else {
        echo "<div id='sysmessage'>"._("Account not found")."</div>"; //account not found by email
    }
} else {
