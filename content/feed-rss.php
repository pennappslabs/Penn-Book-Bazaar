...
<?php 
	if ($resultRSS){
	    foreach ( $resultRSS as $row ){
		    ...
		    if (VIDEO){//erase youtube from RSS
                  $descriptionParts=explode('[youtube=',$postDesc);
                  $postDesc=$descriptionParts[0];
		    }
		    ...
		    ?>
		    <item>
			    <title><![CDATA[<?php echo ucwords($postTypeName)." - ".$postTitle;?>]]></title>
