...
<script type="application/javascript">
	function newAccount(){
		...
		d.email.value = "";
		d.action.value ="new";
		d.submitAccount.value ="<?php echo _("New Account");?>";
		document.getElementById("form-tab").innerHTML ="<?php echo _("New Account");?>";
		show("formAccount");
	...
	function editAccount(account, name, email){
		...
		d.email.value = email;
		d.action.value ="edit";
		d.submitAccount.value ="<?php echo _("Update Account");?>";
		document.getElementById("form-tab").innerHTML ="<?php echo _("Update Account");?>";
		show("formAccount");
		location.href = "#formAccount";
	}	
</script>
<h2><?php echo _("Accounts list");?></h2>
<div id='formAccount' style="display:none;">
	...
	<form name="Account" action="" method="post" onsubmit="return checkForm(this);">
		<fieldset>
			...
			<input id="submitAccount" type="submit" value="<?php echo _("Submit");?>" class="button-submit" />
			<input type="submit" value="<?php echo _("Cancel");?>" class="button-cancel" onclick="hide('formAccount');return false;" />
			<input type="hidden" name="account" value="" />
			<input type="hidden" name="action" value="" />
		</fieldset>
	</form>
</div>
<div class="add_link"><a href="" onclick="newAccount();return false;"><?php echo _("New Account");?></a></div>
<table>
	<tr class="thead">
	...
	<?php 
		if ($_POST){
                ...
                if ($account->exists){
                    ...
                    if ($account->Register($name,$email,$password)){
                        ...
                        if ($account->Activate($token)){
                            $message='<p>'._("Your new account information").'</p>
                            <p><label>'._("Email").': '.$account->email.'</label><br/>
                            <label>'._("Password").': '.$password.'</label></p>';
                            ...
                            sendEmail($email,_("Your new account")." - ".SITE_NAME,$bodyHTML);//
         
                            echo "<div id='sysmessage'>"._("Account succesfully created")."</div>";
                        } else echo "<div id='sysmessage'>"._("An unexpected error has occurred trying to confirm the account")."</div>";
                    } else echo "<div id='sysmessage'>"._("An unexpected error has occurred trying to register the account")."</div>";
                }
            }
            if (is_numeric($account_id)){
                if ($action=="edit"){                
                    ...
                    $message='<p>'._("Your account has been updated").'</p>
                    <label>'._("Name").': '.$name.'</label></p><br/>                
                    <p><label>'._("Email").': '.$email.'</label>';
                    ...
                    sendEmail($email,_("Your account has been updated")." - ".SITE_NAME,$bodyHTML);//
                    
                    echo "<div id='sysmessage'>"._("Account succesfully updated")."</div>";
                    }
                    else echo "<div id='sysmessage'>"._("Account already exists")."</div>";
                }
            }
        } else {
            ...
            if (is_numeric($account_id)){
                if ($action=="activate"){
                    ...
                    $action_notify=_("Activated");
                    
                    echo "<div id='sysmessage'>"._("Account successfully activated")."</div>";
                    ...
                    $action_notify=_("Deactivated");
                    
                    echo "<div id='sysmessage'>"._("Account successfully deactivated")."</div>";
                    ...
                    $action_notify=_("Removed");
                    
                    echo "<div id='sysmessage'>"._("Account successfully removed")."</div>";
                }
                
                if ($action_notify!="" && $email!=""){
                    $message='<p>'._("Your account has been updated").'</p><br/>
                    <label>'._("Actions performed").': '.$action_notify.'</label></p>';
        ...
        $query="SELECT count(a.idAccount) total	FROM ".TABLE_PREFIX."accounts a
                WHERE 1 $filter";
        $total_records = $ocdb->getValue($query,"APP");//query to get total of records
...
?>
	<tr <?php echo $row_class;?>>      
		...
		<td class="action">
        	<a href="" onclick="editAccount('<?php echo $idAccount; ?>', '<?php echo $name;?>', '<?php echo $email;?>');return false;" class="edit"><?php echo _("Edit");?></a> | 
			<?php if ($active){?>
            <a onclick="return confirm('<?php echo _("Deactivate");?>?');" href="?account=<?php echo $idAccount;?>&amp;amail=<?php echo $email;?>&amp;action=deactivate"><?php echo _("Deactivate");?></a> | 
            <?php } else {?>
            <a onclick="return confirm('<?php echo _("Activate");?>?');" href="?account=<?php echo $idAccount;?>&amp;amail=<?php echo $email;?>&amp;action=activate"><?php echo _("Activate");?></a> | 
            <?php }?>
			<a onclick="return confirm('<?php echo _("Delete");?>?');" href="?account=<?php echo $idAccount;?>&amp;amail=<?php echo $email;?>&amp;action=delete" class="delete"><?php echo _("Delete");?></a>
		</td>
	</tr>
	<?php 
			}
...
<form name="ocForm" id="ocForm" action="" method="get">
	<fieldset>
		...
		<input type="submit" value="<?php echo _("Search");?>" class="button-submit" />
	</fieldset>
</form>
<?php
